...
-- ---- ENUM types (use DO block to skip if exists) ----
DO $$ BEGIN CREATE TYPE lead_status AS ENUM ('new', 'contacted', 'qualified', 'nurturing', 'converted', 'lost'); EXCEPTION WHEN duplicate_object THEN null; END $$;
DO $$ BEGIN CREATE TYPE lead_type AS ENUM ('employer', 'recruiter', 'enterprise', 'partner', 'api_user', 'general'); EXCEPTION WHEN duplicate_object THEN null; END $$;
DO $$ BEGIN CREATE TYPE deal_stage AS ENUM ('discovery', 'demo_scheduled', 'demo_done', 'proposal_sent', 'negotiation', 'closed_won', 'closed_lost'); EXCEPTION WHEN duplicate_object THEN null; END $$;
DO $$ BEGIN CREATE TYPE activity_type AS ENUM ('email_sent', 'email_opened', 'email_clicked', 'form_submit', 'page_view', 'api_call', 'report_download', 'demo_request', 'deal_created', 'deal_updated', 'note'); EXCEPTION WHEN duplicate_object THEN null; END $$;
DO $$ BEGIN CREATE TYPE sequence_status AS ENUM ('active', 'paused', 'completed', 'cancelled'); EXCEPTION WHEN duplicate_object THEN null; END $$;
DO $$ BEGIN CREATE TYPE partner_status AS ENUM ('applied', 'approved', 'active', 'suspended', 'churned'); EXCEPTION WHEN duplicate_object THEN null; END $$;
DO $$ BEGIN CREATE TYPE partner_type AS ENUM ('affiliate', 'data', 'platform', 'content', 'reseller'); EXCEPTION WHEN duplicate_object THEN null; END $$;
...
CREATE INDEX IF NOT EXISTS idx_crm_accounts_domain ON crm_accounts(domain);
CREATE INDEX IF NOT EXISTS idx_crm_accounts_country ON crm_accounts(country_code);
CREATE INDEX IF NOT EXISTS idx_crm_accounts_type ON crm_accounts(account_type);
CREATE INDEX IF NOT EXISTS idx_crm_accounts_score ON crm_accounts(enterprise_score DESC);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_crm_leads_email ON crm_leads(email);
...
CREATE INDEX IF NOT EXISTS idx_crm_leads_country ON crm_leads(country_code);
CREATE INDEX IF NOT EXISTS idx_crm_leads_score ON crm_leads(enterprise_score DESC);
CREATE INDEX IF NOT EXISTS idx_crm_leads_source ON crm_leads(source);
CREATE INDEX IF NOT EXISTS idx_crm_leads_last_active ON crm_leads(last_active_at DESC);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_crm_enrollments_unique ON crm_sequence_enrollments(lead_id, sequence_id);
CREATE INDEX IF NOT EXISTS idx_crm_enrollments_status ON crm_sequence_enrollments(status);
...
CREATE INDEX IF NOT EXISTS idx_crm_partners_referral ON crm_partners(referral_code);
CREATE INDEX IF NOT EXISTS idx_crm_partners_status ON crm_partners(status);
CREATE INDEX IF NOT EXISTS idx_crm_partners_type ON crm_partners(partner_type);
...
CREATE INDEX IF NOT EXISTS idx_crm_referrals_partner ON crm_partner_referrals(partner_id);
CREATE INDEX IF NOT EXISTS idx_crm_referrals_code ON crm_partner_referrals(referral_code);
...
CREATE INDEX IF NOT EXISTS idx_crm_api_keys_key ON crm_api_keys(api_key);
CREATE INDEX IF NOT EXISTS idx_crm_api_keys_lead ON crm_api_keys(lead_id);
CREATE INDEX IF NOT EXISTS idx_crm_api_keys_plan ON crm_api_keys(plan);

-- ---- 11. Daily Analytics ----
CREATE TABLE IF NOT EXISTS crm_analytics_daily (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  date DATE NOT NULL UNIQUE,
  new_leads INTEGER DEFAULT 0,
  new_leads_by_type JSONB DEFAULT '{}',
...
CREATE INDEX IF NOT EXISTS idx_crm_analytics_date ON crm_analytics_daily(date DESC);
...
DROP TRIGGER IF EXISTS trg_crm_sequences_updated_at ON crm_email_sequences;
...
-- ---- RLS Policies (anon insert for public forms) ----
DROP POLICY IF EXISTS "anon_insert_leads" ON crm_leads;
CREATE POLICY "anon_insert_leads" ON crm_leads FOR INSERT TO anon WITH CHECK (true);

DROP POLICY IF EXISTS "anon_insert_partners" ON crm_partners;
CREATE POLICY "anon_insert_partners" ON crm_partners FOR INSERT TO anon WITH CHECK (true);

DROP POLICY IF EXISTS "anon_insert_activities" ON crm_activities;
CREATE POLICY "anon_insert_activities" ON crm_activities FOR INSERT TO anon WITH CHECK (true);

-- ---- Done! ----
-- 11 CRM tables created with indexes, triggers, RLS, and policies.
